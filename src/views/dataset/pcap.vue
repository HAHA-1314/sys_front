      ...
      >
        ...
        <el-table-column align="center" prop="name, dir" label="文件名" sortable="">
          <template slot-scope="scope">
            ...
            >
            <el-link
              v-else-if="scope.row.dir === false"
              @click="getStaticFeature(scope.row.name)"
              >{{ scope.row.name }}</el-link
            >
...
export default {
  ...
  methods: {
    ...
    getStaticFeature(filename) {
      let _this = this;
      // post request
      let params = new FormData();
      let _rootdir = this.rootdir;
      params.append("targetDir", _rootdir+"/"+filename);
      this.$axios
        .post("http://211.65.197.130:8099/getStaticFeatureFromPcap", params)
        .then((resp) => {
          console.log(resp);
          if (resp.data.code === 200) {
            _this.$router.push({
